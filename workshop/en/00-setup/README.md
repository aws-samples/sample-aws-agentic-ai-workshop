...
> [!IMPORTANT]
> **The recommended way to run this workshop is to deploy a VS Code Server on AWS.**
> Upload [`code-server.yaml`](../../../code-server.yaml) from the root of this repository to CloudFormation and you get a browser-based development environment with all the tooling installed and this repository already cloned. Just follow **steps 1 through 5** below in order.
>
> If you would rather work on your own laptop, see [Appendix: running on your own machine](#appendix-running-on-your-own-machine). It is not the recommended route, because differences in OS, Python, and fonts can get in the way.

**Estimated time:** ~30 minutes, most of it waiting for CloudFormation
...
<details>
<summary>What you will learn, and the files in this chapter</summary>

**What you will learn**

- How to deploy and open the workshop VS Code Server (code-server) environment with CloudFormation
- How to install the workshop Python environment with `uv` and run a lab file from the repository root
- How to enable Amazon Bedrock model access in `us-west-2`
- What the workshop's `create-uv-env.sh` script does
...
| File | Purpose |
|---|---|
| `code-server.yaml` (repository root) | the CloudFormation template that builds the workshop environment: an EC2-backed code-server plus a CloudFront distribution |
...
Sign in to the AWS console as an IAM user with administrator privileges. If you already have one, use it and skip to [step 2](#2-deploy-the-code-server-cloudformation-stack).

<details>
<summary>Creating an AWS account and an Administrator IAM user</summary>
...
## 2. Deploy the code-server CloudFormation stack

### 2-1. Get the template file

The CloudFormation template **`code-server.yaml` is included in the root of this repository.** Download it to your machine in one of these ways:

- **From a terminal**

  ```bash
  curl -LO https://raw.githubusercontent.com/aws-samples/sample-aws-agentic-ai-workshop/main/code-server.yaml
  ```

- **From the browser**: open [code-server.yaml](https://raw.githubusercontent.com/aws-samples/sample-aws-agentic-ai-workshop/main/code-server.yaml), then right-click and choose **Save as**. Make sure the file keeps the `.yaml` extension.
- **If you already cloned the repository**: use the `code-server.yaml` at the root of your clone.
...
2. Select **Choose an existing template** → **Upload a template file**, click **Choose file** to upload the `code-server.yaml` you just downloaded, and click **Next**.
...
4. Leave **every parameter at its default**. The defaults clone this repository into `/workshop` on the EC2 instance and open that folder in code-server.
5. Click **Next** through to the last page, check the box acknowledging that the stack creates IAM resources (**I acknowledge that AWS CloudFormation might create IAM resources**), and click **Submit**.

> [!NOTE]
> The stack takes about 10 minutes or more to finish. While you wait for `CREATE_COMPLETE`, work through [step 3](#3-enable-amazon-bedrock-model-access).
...
1. Go to [CloudFormation](https://us-east-1.console.aws.amazon.com/cloudformation/home) in the AWS console and confirm the `code-server-python` stack is in `CREATE_COMPLETE`.
2. Open the **Outputs** tab, copy the code-server `Password`, then open the `URL` from the same tab and paste the password in.
...
3. You should see this screen, with the workshop repository already open.
...
Run these commands in the code-server terminal. The repository is cloned at `/workshop`, and the lab code lives in its `code/` folder. uv is installed as part of the setup.
...
When it finishes, verify the environment from the repository root:

```bash
uv run --project 00-setup python 01-single-agent/completed/basic.py
```

If that prints an agent response, the environment and your Bedrock access are both working. Continue to [chapter 01](../01-single-agent/README.md).

<details>
<summary>What create-uv-env.sh does</summary>
...
The workshop assumes broad permissions, and several steps create IAM roles. In a personal sandbox account the simplest option is to attach **AdministratorAccess** to the user you work as, which is what the account setup in step 1 does.
...
Nothing in this chapter creates a billable resource beyond the CloudFormation stack from `code-server.yaml`, which runs an EC2-backed code-server environment for as long as the stack exists. Delete the stack when you finish the workshop.
...
## Appendix: running on your own machine

> [!NOTE]
> This section is the fallback for when you cannot use the CloudFormation environment. Prefer [steps 1 to 5](#1-prepare-an-aws-account) above.
...
### 2. Clone the repository and configure AWS credentials

```bash
git clone https://github.com/aws-samples/sample-aws-agentic-ai-workshop.git
cd sample-aws-agentic-ai-workshop/code
aws configure
```
...
Then enable model access as described in [3. Enable Amazon Bedrock model access](#3-enable-amazon-bedrock-model-access).
...
**The CloudFormation stack fails to create**
Open the **Events** tab in the console and read the message on the first `CREATE_FAILED` event. Two causes are common. First, the IAM-resource acknowledgement checkbox was not selected. Second, the default instance type (`t4g.medium`) is not available in your region — delete the stack and redeploy with `InstanceType` set to something like `t3.medium`.

**The code-server URL does not open, or the password is rejected**
The CloudFront distribution can take a few more minutes to propagate. If the URL still does not open after the stack reaches `CREATE_COMPLETE`, wait a moment and retry, and check that you pasted the `Password` output with no leading or trailing whitespace.
...
See [Optional: Korean fonts for matplotlib](#5-optional-korean-fonts-for-matplotlib).
