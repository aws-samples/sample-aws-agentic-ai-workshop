...
> [!IMPORTANT]
> **이 워크샵은 AWS에 VS Code Server를 배포해서 진행하는 것을 권장합니다.**
> 이 리포지토리 루트의 [`code-server.yaml`](../../../code-server.yaml)을 CloudFormation에 업로드하면, 필요한 도구가 모두 설치되고 이 리포지토리가 미리 클론된 브라우저 개발 환경이 만들어집니다. 아래 **1단계부터 5단계까지 순서대로** 따라가시면 됩니다.
>
> 본인 노트북에서 실습하고 싶다면 [부록: 본인 컴퓨터에서 진행하기](#부록-본인-컴퓨터에서-진행하기)를 참고하세요. 다만 OS·Python·폰트 차이로 인한 문제가 생길 수 있어 권장하지 않습니다.

**예상 소요 시간:** 약 30분 (대부분 CloudFormation 대기 시간)
...
<details>
<summary>이 챕터에서 배우는 내용과 파일 목록</summary>
...
| 파일 | 용도 |
|---|---|
| `code-server.yaml` (리포지토리 루트) | 워크샵 실습 환경을 만드는 CloudFormation 템플릿. EC2 기반 code-server와 CloudFront 배포를 생성합니다 |
...
<details>
<summary>AWS 계정과 Administrator IAM 사용자 만들기</summary>
...
## 2. code-server CloudFormation 스택 배포

### 2-1. 템플릿 파일 준비

CloudFormation 템플릿 **`code-server.yaml`은 이 리포지토리 루트에 포함되어 있습니다.** 아래 방법 중 하나로 본인 컴퓨터에 내려받으세요.
...
- **브라우저에서 다운로드**: [code-server.yaml](https://raw.githubusercontent.com/aws-samples/sample-aws-agentic-ai-workshop/main/code-server.yaml) 링크를 열고 마우스 오른쪽 클릭 → **다른 이름으로 저장**을 선택합니다. 파일 확장자가 `.yaml`로 유지되는지 확인하세요.
- **리포지토리를 이미 클론한 경우**: 클론한 폴더 루트의 `code-server.yaml`을 그대로 사용합니다.
...
2. **Choose an existing template** → **Upload a template file**을 선택한 뒤, **Choose file**을 클릭해 방금 내려받은 `code-server.yaml`을 업로드하고 **Next**를 클릭합니다.
...
4. 파라미터는 **모두 기본값 그대로** 둡니다. 기본값에서 이 리포지토리가 EC2 인스턴스의 `/workshop` 폴더로 자동 클론되고, code-server가 해당 폴더를 열도록 설정됩니다.
5. **Next**를 계속 눌러 마지막 페이지로 이동한 뒤, IAM 리소스 생성에 동의한다는 체크박스(**I acknowledge that AWS CloudFormation might create IAM resources**)를 선택하고 **Submit**을 클릭합니다.

> [!NOTE]
> CloudFormation 스택 생성은 약 10분 이상 소요됩니다. 스택이 `CREATE_COMPLETE`가 될 때까지 기다리는 동안 [3단계](#3-amazon-bedrock-모델-액세스-활성화)를 진행하세요.
...
1. AWS 콘솔에서 [CloudFormation](https://us-east-1.console.aws.amazon.com/cloudformation/home)으로 이동한 뒤 `code-server-python` 스택이 `CREATE_COMPLETE` 상태인지 확인합니다.
2. **Outputs** 탭을 눌러 code server의 `Password`를 복사하고, 같은 탭의 `URL`에 접속해 복사한 password를 붙여넣습니다.
...
3. 아래 화면을 확인합니다. 워크샵 리포지토리가 이미 열려 있어야 합니다.
...
아래 명령어를 code-server 터미널에 입력합니다. 리포지토리는 `/workshop`에 클론되어 있고 실습 코드는 그 아래 `code/` 폴더에 있습니다. 세팅을 위한 uv가 설치됩니다.
...
설치가 끝나면 리포지토리 루트에서 다음 명령으로 확인합니다.
...
워크샵은 폭넓은 권한을 전제로 하며, 일부 단계에서 IAM 역할을 생성합니다. 개인 샌드박스 계정이라면 실습에 사용할 사용자에게 **AdministratorAccess**를 연결하는 것이 가장 간단하며, 1단계의 계정 설정 과정도 그렇게 진행합니다.
...
이 챕터에서 과금이 발생하는 리소스는 `code-server.yaml`이 만드는 CloudFormation 스택뿐입니다. 이 스택은 EC2 기반 code-server 환경을 구성하므로 스택이 존재하는 동안 계속 과금됩니다. 워크샵을 마치면 스택을 삭제하세요.
...
> [!NOTE]
> 이 섹션은 CloudFormation 환경을 사용할 수 없는 경우를 위한 대안입니다. 가능하면 위의 [1~5단계](#1-aws-계정-준비)를 따르시기 바랍니다.
...
이어서 [3. Amazon Bedrock 모델 액세스 활성화](#3-amazon-bedrock-모델-액세스-활성화)의 안내에 따라 모델 액세스를 활성화합니다.
...
**CloudFormation 스택 생성이 실패합니다**
콘솔의 **Events** 탭에서 첫 번째 `CREATE_FAILED` 이벤트의 메시지를 확인하세요. 자주 발생하는 원인은 두 가지입니다. 첫째, IAM 리소스 생성 동의 체크박스를 선택하지 않은 경우입니다. 둘째, 리전에서 기본 인스턴스 타입(`t4g.medium`)을 사용할 수 없는 경우로, 이때는 스택을 삭제하고 `InstanceType` 파라미터를 `t3.medium` 등으로 바꿔 다시 배포하세요.

**code-server URL에 접속되지 않거나 로그인이 되지 않습니다**
CloudFront 배포가 전파되기까지 몇 분이 더 걸릴 수 있습니다. 스택이 `CREATE_COMPLETE`가 된 뒤에도 접속되지 않으면 잠시 기다린 뒤 다시 시도하고, **Outputs** 탭의 `Password` 값을 앞뒤 공백 없이 붙여넣었는지 확인하세요.
...
[(선택) matplotlib 한글 폰트 설정](#5-선택-matplotlib-한글-폰트-설정)을 참고하세요.
